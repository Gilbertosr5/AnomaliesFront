import { PredictFile, PredictJSON } from "../types/predictsTypes";
import api from "./api";
...
export async function getPredictCSV(file: File){
  try{
    ...
    const responseData: PredictFile[] = response.data;
    console.log("Response PredictCSV =>", response.data);
    console.log("Response PredictCSV inputData =>", responseData[0].input_data);
    console.log("Response PredictCSV result =>", responseData[0].result);
    return response.data;
  }catch(error){
...
export async function getPredictParquet(file: File){
  try{
    const formData = new FormData();
    formData.append("file", file);
    const response = await api.post("predict/parquet", formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
